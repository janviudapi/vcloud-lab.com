...
resource "azurerm_key_vault_access_policy" "appgw_access_policy" {
  ...
  depends_on = [
    azurerm_role_assignment.kv_certificate_role_assignment_contributor
  ]
}
...
resource "azurerm_role_assignment" "kv_certificate_role_assignment_contributor" {
  principal_id         = azurerm_user_assigned_identity.appgw_identity.principal_id
...
resource "azurerm_application_gateway" "appgw" {
  ...
  ssl_certificate {
    ...
    key_vault_secret_id = azurerm_key_vault_certificate.testcert.secret_id #data.azurerm_key_vault_certificate.keyvaultcertificateinfo.secret_id
  }
